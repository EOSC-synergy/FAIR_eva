...
There are some test whose results depend on things outside of the metadata given by the EPOS API so their result depends on a configuration parameter. These parameters are stored in the file 'config.ini' you can chenge this parameters to change some resutlts. WARNING a lot of parameters are essential for the tool to work if the parameter you are interested in changing doesn't appear on the following list you probably shouldn't change it:

1. supportted_data_formats: The base fromats that are standard.
...
6. metadata_standard: The standard the metadata is based on
...
There are some parameters with * this mean that at the time of writing we have not found (mainly becaiuse it doesn't exist at the moment or is not clear) a good parameter.
