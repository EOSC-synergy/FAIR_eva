...
class Digital_CSIC(Evaluator):
    ...
    def __init__(self, item_id):
        ...
        try:
            self.connection = psycopg2.connect(user=config['digital_csic']['db_user'],
                    password=config['digital_csic']['db_pass'], host=config['digital_csic']['db_host'], port=config['digital_csic']['db_port'],
                    database=config['digital_csic']['db_db'])
        except (Exception, psycopg2.Error) as error:
            print ('Error while fetching data from PostgreSQL', error)
        self.internal_id = self.get_internal_id(self.item_id, self.connection)
        if self.id_type == "doi":
            self.handle_id = self.get_handle_id(self.internal_id, self.connection)
   ...
   # rda_f1_02m from parent
   # rda_f1_02d from parent
    ...
    # Checkin Dublin Core
        ...
        if len(dc_terms[1]) == sum_array:
            ...
            for e in dc_terms[1]:
                ...
                i = i + 1
            points = 100 * (len(dc_terms[1]) - missing_elements) / len(dc_terms[1])
    ...
    def rda_a1_03m(self):
        ...
        points = 0
        msg = "Provided ID does not resolve to DIGITAL.CSIC. This test need to be improved"
        ...
        if self.id_type == "doi":
            ...
            if response.history:
                ...
                if landing_url in response.url:
                    points = 100
                    msg = "Your Unique identifier is a Handle PID and redirects correctly to DIGITAL.CSIC"
    ...
    def rda_a1_04m(self):
        ...
        msg = "(Meta)data is not accessible using the identifier. Please, check with DIGITAL.CSIC"
        return points, msg
    ...
    def rda_a1_04m(self):
        ...
        if points == 100:
            msg = msg + '. Accessible via HTTP protocol.'
        msg = "Data is not accessible using the identifier. Please, check with DIGITAL.CSIC"
        return points, msg
    ...
    def rda_a1_05d(self):
        ...
        """
        points = 0
        msg = "DIGITAL.CSIC does not currently provides an automatic protocol to retrieve the digital object"
        return points, msg
    ...
    def rda_a1_2_01d(self):
        ...
        """
        points = 100
        msg = "DIGITAL.CSIC allows restricted access to digital object using institutional AAI"
        return points, msg
    ...
    def rda_a2_01m(self):
        ...
        """
        points = 100
        msg = "DIGITAL.CSIC preserves the metadata even if the digital object is deleted"
        return points, msg
    ...
    def rda_i1_01d(self):
        ...
        standard_list = ["pdf", "csv", "jpg", "jpeg", "nc", "hdf", "mp4", "mp3", "wav", "doc", "txt", "xls", "xlsx", "sgy","zip"]
        
        query = "SELECT bitstream.name FROM item2bundle, bundle2bitstream, bitstream WHERE item2bundle.item_id = '%s' AND item2bundle.bundle_id = bundle2bitstream.bundle_id AND bundle2bitstream.bitstream_id = bitstream.bitstream_id" % self.internal_id
        ...
        if points == 0:
            msg = "The digital object is not in an accepted standard format. If you think the format should be accepted, please contact DIGITAL.CSIC"
        elif points < 100:
    ...
    def rda_i1_02m(self):
        ...
        try:
            xmlTree = ET.fromstring(oai.text)
            msg = 'Metadata can be extracted using machine-actionable features (XML Metadata)'
            points = 100
    ...
    def rda_i2_01m(self):
        ...
        query = "SELECT DISTINCT metadataschemaregistry.namespace, metadataschemaregistry.short_id FROM metadatavalue, metadatafieldregistry, metadataschemaregistry WHERE metadatavalue.item_id = %s AND metadatavalue.metadata_field_id = metadatafieldregistry.metadata_field_id AND metadatafieldregistry.metadata_schema_id = metadataschemaregistry.metadata_schema_id" % self.internal_id
        cursor = self.connection.cursor()
        cursor.execute(query)
        ...
        if points == 0:
            msg = "The metadata standard documentation can not be retrieved. Schema(s): %s" % schemas
        elif points < 100:
            msg = "Some of the metadata schemas used are not accessible via persistent identifier. Schema(s): %s" % schemas
    ...
    def rda_i3_01m(self):
        ...
        if orcids > 0 or pids > 0:
            points = 100
            msg = "Your (meta)data includes %i references to other digital objects and %i references for contributors. Do you think you can improve that information?" % (pids, orcids)
    ...
    """
        ...
        if orcids > 0 or pids > 0 or dois > 0:
            points = 100
            msg = "Your (meta)data includes %i qualified references to other digital objects and %i references to contributors. Do you think you can improve that information?" % (pids + dois, orcids)
    ...
    def rda_r1_2_01m(self):
        ...
        """
        points = 0
        msg = "Currently, DIGITAL.CSIC does not include community-bsed schemas. If you need to include yours, please contact."
        return points, msg
...
#UTILS
    def get_internal_id(self, item_id, connection):
        ...
        if len(id_to_check) != 0:
            if self.check_doi(id_to_check):
                query = "SELECT item.item_id FROM item, metadatavalue, metadatafieldregistry WHERE item.item_id = metadatavalue.item_id AND metadatavalue.metadata_field_id = metadatafieldregistry.metadata_field_id AND metadatafieldregistry.element = 'identifier' AND metadatavalue.text_value LIKE '%s'" % temp_str
                print(query)
    ...
    def get_handle_id(self, internal_id, connection):
        query = "SELECT metadatavalue.text_value FROM item, metadatavalue, metadatafieldregistry WHERE item.item_id = %s AND item.item_id = metadatavalue.item_id AND metadatavalue.metadata_field_id = metadatafieldregistry.metadata_field_id AND metadatafieldregistry.element = 'identifier' AND metadatafieldregistry.qualifier = 'uri'" % internal_id
        cursor = connection.cursor()
        cursor.execute(query)
        list_id = cursor.fetchall()
        handle_id = ''
